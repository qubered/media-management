...
- **`npm run typecheck` fails on a completely fresh checkout** with
  `Cannot find name 'LayoutProps'` — that type is generated by Next itself
  into `.next/types/`, not something `tsc` alone can see. Run
  `npx next typegen` (or just `npm run dev`/`npm run build` once) before
  typechecking if `.next/` doesn't exist yet. CI does this explicitly; do
  the same locally after a clean clone or an `rm -rf .next`.
